...
def _add_timezone_policy(parser):
    ...
    parser.add_argument(
        "--fold-policy", choices=("error", "earlier", "later"), default="error"
    )
    parser.add_argument(
        "--gap-policy", choices=("error", "next", "previous"), default="error"
    )
...
def _build_parser(command):
    ...
    if command == "tour":
        ...
        parser.add_argument(
            "--dry-run",
            action="store_true",
            help="Print the editor command without launching it.",
        )
        parser.add_argument(
            "--review-only",
            action="store_true",
            help="Open a temporary copy and print the diff without applying it.",
        )
        parser.add_argument(
            "--reconcile",
            action="store_true",
            help="Conservatively merge non-overlapping external changes made while the editor is open.",
        )
        parser.add_argument(
            "--keep-temp",
            action="store_true",
            help="Keep the edited temporary copy for manual recovery.",
        )
        parser.add_argument(
            "--show-diff",
            action="store_true",
            help="Print the applied diff after a successful edit.",
        )
    elif command == "path":
        parser.add_argument("--format", choices=("text", "json"), default="text")
        parser.add_argument("--pretty", action="store_true")
        ...
        parser.add_argument(
            "--round-minutes", "--round", dest="round_minutes", type=int, default=0
        )
        parser.add_argument("--currency", default="JPY")
        parser.add_argument(
            "--format", choices=("text", "markdown", "csv", "json"), default="text"
        )
        parser.add_argument("--pretty", action="store_true")
        parser.add_argument("-o", "--output")
        ...
        parser.add_argument(
            "--format", choices=("text", "markdown", "slack", "json"), default="text"
        )
        parser.add_argument("--pretty", action="store_true")
        parser.add_argument("-o", "--output")
    elif command == "to-ics":
        ...
        parser.add_argument(
            "--due-soon-days", "--due-soon", dest="due_soon_days", type=int, default=7
        )
        parser.add_argument("--format", choices=("text", "json"), default="text")
        parser.add_argument("--pretty", action="store_true")
    elif command == "review":
        ...
        parser.add_argument(
            "--revision", help="Expected SHA-256 revision of the journal target."
        )
        parser.add_argument("--dry-run", action="store_true")
        ...
        for action in (
            ...
            if action not in ("status", "open"):
                child.add_argument(
                    "--id",
                    required=True,
                    help="Item ID receiving the attachment reference.",
                )
            child.add_argument(
                "--file",
                required=True,
                help="Stored attachment path relative to life.txt.",
            )
            if action == "put":
                child.add_argument(
                    "--source",
                    required=True,
                    help="Source file copied into the attachment target with a bounded reader.",
                )
            ...
            if action == "package":
                child.add_argument(
                    "--source",
                    required=True,
                    help="Source directory packaged as a deterministic ZIP attachment.",
                )
            ...
            if action == "reconcile":
                ...
                child.add_argument(
                    "--recorded-revision",
                    help="Hash currently stored on the item; stale values are rejected.",
                )
            if action == "open":
                child.add_argument(
                    "--metadata-revision", help="Expected open-metadata state revision."
                )
                child.add_argument(
                    "--no-record",
                    action="store_true",
                    help="Do not update the local open-reference metadata state.",
                )
                child.add_argument(
                    "--execute",
                    action="store_true",
                    help="Run the platform opener after validation. Default only returns the command plan.",
                )
            child.add_argument(
                "--item-revision", help="Expected life.txt SHA-256 revision."
            )
            child.add_argument(
                "--attachment-revision",
                help="Expected attachment SHA-256 revision or <missing>.",
            )
            child.add_argument(
                "--transaction-id",
                help="Stable transaction id for retry/restart recovery.",
            )
            child.add_argument(
                "--require-revisions",
                action="store_true",
                help="Reject missing required revisions.",
            )
        ...
        delegated.add_argument(
            "delegated_action", choices=("prepare", "inspect", "apply", "reject")
        )
        delegated.add_argument(
            "--path", help="Authoritative life.txt path for prepare."
        )
        delegated.add_argument(
            "--proposal", required=True, help="Persistent delegated proposal JSON path."
        )
        delegated.add_argument(
            "--command", help="Command for prepare; use {file} for the temporary copy."
        )
        ...
        transactions.add_argument(
            "--restore-action",
            choices=("inspect", "resume", "compensate"),
            default="inspect",
        )
