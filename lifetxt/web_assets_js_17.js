    ...
    //
    // Declared here at true top level -- not inside the loadConfig().then()
    // callback below -- because the record-detail drawer editor (#850, in
    // web_assets_js_12.js) calls renderStructuredFields()/_structuredDetails()/
    // syncStatusStateMode() directly by name from top-level code, which has
    // no lexical access into that callback's own function scope. Everything
    // still reachable only from inside the callback (the main editor's own
    // wiring, refreshStructuredFields(), etc.) stays where it was.
    ...
    // Actionable/retryable error presentation (#798). Declared here at top
    // level for the same reason as the structured-field helpers above: both
    // the drawer editor (web_assets_js_12.js) and the main editor
    // (web_assets_js_05.js) call showActionableError() directly by name from
    // outside the loadConfig().then() callback below.
