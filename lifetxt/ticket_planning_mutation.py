...
def _apply_text(path, expected_revision, transform, operation, dry_run=False):
    ...
    if snapshot.content_hash != expected:
        raise mutation.MutationConflict(
            path, expected, snapshot.content_hash, operation=operation
        )
    holder = {}
    ...
    if not dry_run:
        result = mutation.mutate_text(
            path,
            lambda current: transform(current, holder),
    ...
    return OrderedDict(
        (
            ...
            (
                "record",
                holder.get("record").to_dict() if holder.get("record") else None,
            ),
        )
    )
...
def create_version(
        ...
        if parent:
            parent_item = next(
                (
                    ...
                    if str(_first(item, "id", "")) == str(parent)
                ),
                None,
            )
            ...
            if str(_first(parent_item, "project", "")) != str(project):
                raise ValueError(
                    "Parent version %r belongs to another project." % parent
                )
    ...
    return _apply_text(
        path, expected_revision, transform, "version.new", dry_run=dry_run
    )
...
def create_sprint(
        ...
        if version:
            version_item = next(
                (
                    item
                    for item in iter_versions(items)
                    if str(_first(item, "id", "")) == str(version)
                ),
                None,
            )
    ...
    return _apply_text(
        path, expected_revision, transform, "sprint.new", dry_run=dry_run
    )
...
def update_planning_state(
    ...
    def transform(text, holder):
        ...
        if current_state == state:
            raise ValueError(
                "%s %s is already in state %s." % (marker, identifier, state)
            )
        transitions = (
            {
        ...
        replacement, updated = _replace_record(
            ...
            identifier,
            marker,
            lambda item: item.details.__setitem__("state", [state]),
            key,
        )
    ...
    return _apply_text(
        path, expected_revision, transform, "%s.%s" % (marker, state), dry_run=dry_run
    )
...
def assign_planning(
    ...
    if clear_version:
        ...
        if (
            ticket_project
            and str(_first(version_item, "project", "")) != ticket_project
        ):
            raise ValueError("Version %r belongs to another project." % version)
    ...
    if clear_sprint:
        ...
        sprint_item = next(
            (
                ...
                if str(_first(item, "id", "")) == str(sprint)
            ),
            None,
        )
