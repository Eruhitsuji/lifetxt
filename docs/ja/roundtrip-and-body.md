...
インラインの `body:` detailと `|` 継続行を組み合わせてはいけません。
...
この継続行は「インライン値へ追記する」とも「別のbody値を作成する」とも解釈できるため、パーサーは継続行の位置にエラー `E022` を報告します。

すべてが1行である繰り返しbody値は有効です。
...
- すべてが1行の場合に限り、インラインの `body:` を繰り返す
- 複数行値を1つの継続ブロックで記述する

同じ項目でインライン形式と継続形式を混在させないでください。このfail-loud規則により、フォーマッター、コンバーター、エディター連携、将来のLSP操作がデータモデルを暗黙に変更することを防ぎます。
