...
def format_summary(report: Mapping[str, Any]) -> str:
    ...
    lines = [
        "Tickets: {total} total, {open} open, {terminal} terminal, {progress}% count progress".format(
            ...
            else summary["progress_percent"],
            **summary,
        ),
        "Attention: {blocked} blocked, {dependency_unknown} dependency unknown, {overdue} overdue, "
        "{unassigned} unassigned, {high_severity} high severity, {stale} stale".format(
            **summary
        ),
        "",
    ...
    for project in report["projects"]:
        progress = (
            "-"
            if project["progress_percent"] is None
            else "%.2f%%" % project["progress_percent"]
        )
        lines.append(
            "{project} | {open}/{total} | {progress} | {blocked} | {dependency_unknown} | {overdue} | "
            "{unassigned} | {high_severity} | {stale}".format(
...
def format_board(report: Mapping[str, Any]) -> str:
    ...
    for status, rows in report["board"].items():
        ...
        for row in rows:
            ...
            lines.append(
                "- [%s] %s — %s%s%s"
                % (row["id"] or "(no-id)", row["project"], row["title"], due, assignee)
            )
        lines.append("")
    return "\n".join(lines).rstrip() + "\n"
...
def format_attention(report: Mapping[str, Any]) -> str:
    ...
    for category, rows in report["attention"].items():
        ...
        lines.extend(
            "- [%s] %s — %s" % (row["id"] or "(no-id)", row["project"], row["title"])
            for row in rows
        )
        lines.append("")
    return "\n".join(lines).rstrip() + "\n"
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        prog="python -m lifetxt.ticket_projects",
        description="Read-only ticket/project reports.",
    )
    ...
    parser.add_argument(
        "--stale-after", type=int, default=DEFAULT_STALE_DAYS, metavar="DAYS"
    )
    parser.add_argument(
        "--terminal-status",
        action="append",
        dest="terminal_statuses",
        help="Repeat to replace built-in terminal statuses.",
    )
    parser.add_argument(
        "--high-severity",
        action="append",
        dest="high_severities",
        help="Repeat to replace built-in high severities.",
    )
...
def main(argv: Optional[Sequence[str]] = None) -> int:
    ...
    if args.format == "json":
        json.dump(
            report, sys.stdout, ensure_ascii=False, indent=2 if args.pretty else None
        )
        sys.stdout.write("\n")
    else:
        sys.stdout.write(
