...
`http://127.0.0.1:8000/capture` のように `/capture` を開くと、記録に必要な
操作だけを表示する画面へ直接移動できます。この URL をブックマークするか、スマート
フォンのホーム画面へ追加してください。Web UI 全体と同じ正規のクイック追加経路と
省略記法（`@project`、`#tag`、`!priority`、`^due`、または完全な life.txt 行）を
使用します。成功時は入力を消去して再度フォーカスし、失敗時は再試行できるよう入力
内容を保持します。

Web UI はアプリマニフェストとスマートフォン向けサイズの life.txt アイコンを配信します。
対応するモバイルブラウザで `/capture` を表示し、**ホーム画面に追加**または**インストール**
を選ぶと、ホーム画面のショートカットからクイックキャプチャを直接開けます。対応する
プラットフォームでは単独のアプリ表示になります。

クイックキャプチャはオンライン専用です。オフラインキャッシュや変更キューは追加せず、
Web サーバーの `no-store`、認証、読み取り専用、書き込み先の規則も変更しません。

自動モバイル検証では、headless Chrome 上で 320、360、390、430 CSS px の縦画面と、
横向きの低い画面、ソフトウェアキーボード表示を想定した低い viewport を確認します。
横 overflow、タッチ対象サイズ、初回・再送後の focus、キーボード送信、成功・失敗表示、
重複送信抑止、英語・日本語表示が対象です。これはブラウザエミュレーションであり、実機
...
- **Now / 現在時刻**：タスクを完了にし、現在日時を秒精度で `done` に保存します。
- **Custom / 指定して完了**：日付を選んで完了します。**時刻（任意）** を空欄にすると `done:YYYY-MM-DD`、時刻を入力すると明示的な UTC 日時として保存します。例えば Asia/Tokyo の 2026-09-19 00:15 は `done:2026-09-18T15:15:00Z` になります。
- **キャンセル**・Escape・保存前に詳細を閉じる操作では、完了状態も `done` も変更しません。

時刻入力には画面に表示されるブラウザーのタイムゾーンを使います。
日時は UTC で保存し、日付のみの場合は選んだ日付をそのまま保存します。
Custom の時刻入力は分単位です。夏時間への切り替えで存在しない時刻は
保存せず、時刻が重複する場合はブラウザーの仕様に従い先の時刻を選びます。
Now は端末の時計を使います。既存の `done` がある場合は値と上書きの説明を表示し、
Now または Custom の保存を明示的に選んだ場合だけ置き換えます。
取り消し（Undo）では、元の状態と詳細情報を復元します。

従来の **完了** ボタンも使用でき、既存の `done` を維持します
（未設定なら日付を追加しません）。
**Complete + repeat**・一括完了・CLI/TUI の操作は従来どおりです。
