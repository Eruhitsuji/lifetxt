...
class BatchFollowupTests(unittest.TestCase):
    ...
    def test_context_capsule_cutoff_is_explicit(self):
        ...
        self.assertEqual(
            ...
            context_capsule(items, evaluation_time="2020-02-01T00:00:00+00:00")[
                "schema"
            ],
        )
    ...
    def test_decisions_and_feed_have_stable_schemas(self):
        items, _ = parse_text(
            '[ ] N "Decision" tag:decision id:d1\n[x] T "Actual" id:a1 realizes:d1\n'
        )
        result = decision_outcome_review(items)
        self.assertEqual("decision-outcome-review-v1", result["schema"])
        self.assertEqual("realized", result["decisions"][0]["outcome"])
        self.assertEqual(
            "temporal-change-feed-v1",
            temporal_change_feed(items, "2030-01-01T00:00:00+00:00")["schema"],
        )
