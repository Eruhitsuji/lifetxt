...
class TicketProjectReportTests(unittest.TestCase):
    ...
    def test_attention_categories_are_transparent_and_non_exclusive(self):
        items = [ticket(
            "Security blocker", "T-1", ticket_status="blocked", severity="critical",
            due="2026-07-20", updated="2026-06-01T00:00:00Z",
        )]
    ...
    def test_date_only_due_is_not_overdue_until_next_utc_midnight(self):
        report = build_ticket_project_report([ticket("Due today", "T-1", due="2026-07-25")], reference_time=self.now)
        self.assertFalse(report["tickets"][0]["overdue"])
        after_midnight = datetime(2026, 7, 26, 0, 0, tzinfo=timezone.utc)
        report = build_ticket_project_report([ticket("Due yesterday", "T-1", due="2026-07-25")], reference_time=after_midnight)
    ...
    def test_closed_dependency_does_not_block(self):
        items = [ticket("Dependency", "T-1", ticket_status="closed"), ticket("Dependent", "T-2", depends_on="T-1")]
        report = build_ticket_project_report(items, reference_time=self.now)
        dependent = next(row for row in report["tickets"] if row["id"] == "T-2")
    ...
    def test_missing_dependency_is_reported_without_guessing_blocked_state(self):
        report = build_ticket_project_report([ticket("Dependent", "T-2", depends_on="T-missing")], reference_time=self.now)
        row = report["tickets"][0]
    ...
    def test_estimate_elapsed_coverage_and_variance(self):
        items = [
            ticket("Measured", "T-1", estimate="1d", elapsed="10h"),
            ticket("Estimate only", "T-2", estimate="2h"),
            ticket("Invalid", "T-3", estimate="soon", elapsed="1h"),
        ]
    ...
    def test_board_order_is_deterministic(self):
        items = [
            ticket("Low", "T-2", priority="low"), ticket("High", "T-1", priority="high"),
    ...
    def test_terminal_and_severity_sets_are_configurable(self):
        ...
        report = build_ticket_project_report(
            items, reference_time=self.now, terminal_statuses=["shipped"], high_severities=["sev1"],
        )
    ...
    def test_report_matches_published_schema(self):
        ...
        report = build_ticket_project_report([
            ticket("Open", "T-1", depends_on="missing", estimate="2h", elapsed="3h"),
