...
Last updated: 2026-07-24 (updated x101)
...
- [ ] Expand the schema bundle beyond the current 31 documents where real contracts exist. Semantic write results, archive operations, attachment transactions, compound work sessions, transaction policy, backup integrity, fault-injection reports, clock-boundary audit, versioned configuration (`config-v1`), and workspace source manifests (`workspace-source-manifest-v1`) documents are now published and validated. Next add JSONL record schemas, endpoint-specific Web request/response schemas, MCP tool/resource result schemas, query language, project registry metadata, normalized integration events, provider operation results, notification backend results, directory attachment packages, policy migration results, and import/export manifests. Generate schemas from authoritative registries where possible and validate real responses rather than hand-written examples alone.
...
The foundation now exists in [`lifetxt/workspace.py`](lifetxt/workspace.py), [`lifetxt/config_layers.py`](lifetxt/config_layers.py), and [`lifetxt/config_registry.py`](lifetxt/config_registry.py): a versioned source-manifest model (`path`, `role`, `required`, `writable`, `default_visible`, `format`, `priority`, `watch`, `privacy`, `generated_by`, `exclude`) with legacy `paths`/`write_file` treated as an implicit `workspaces.default`; a tested precedence model (built-in defaults → config file → profile → environment → CLI flags) with provenance; named workspaces and profiles with an explicit `default_workspace`; relative-path resolution against the configuration directory with deterministic glob order and WS001-WS010 diagnostics; the global `--workspace NAME` flag; `lifetxt workspace list|show|files|validate`; `lifetxt config effective|sources|get|set|unset|explain`; `config-v1.schema.json` and `workspace-source-manifest-v1.schema.json`; runnable `examples/config/*` validated against the schema; and English/Japanese `docs/*/config.md`. Remaining work extends and hardens that foundation.

- [ ] Enforce `config_version` semantics on top of the published `config-v1.schema.json`: add deprecation metadata, restart-required annotations, and migration rules to the registry, and reject unsupported future versions for writes while allowing read-only inspection and diagnostics.
- [ ] Extend workspace resolution edge cases: detect symlink loops, path aliases pointing at the same physical file through different links, generated/archive write targets, and excessive source counts or total size. Surface each as a stable WS diagnostic and cover it with one-winner tests.
- [ ] Add the remaining config/workspace subcommands: `config check`, `config migrate`, and `workspace doctor`. `workspace files --resolved` should additionally report the matched glob and the include/exclude reason for every file.
- [ ] Route `config set`/`config unset`/migrations and Web settings through a revision-aware atomic config writer that validates against `config-v1.schema.json` before commit, preserves permissions, retains a bounded backup/recovery record, and rejects stale or unsupported-version updates. (Current writes are atomic but not yet schema-validated or CAS-guarded.)
- [ ] Enforce the credential policy beyond redaction: reject committed plaintext tokens/passwords for supported secret fields at validation time and require environment-variable or OS credential references. Redaction already covers effective config, sources, and support bundles.
...
- [ ] Add schema-valid examples for generated-calendar, team, remote, kiosk, and integration workspaces beyond the current personal/work/project set. Test precedence, migration, Windows/POSIX paths, Unicode, broken JSON, unknown/deprecated keys, cycles, missing files, glob order, and rollback after interrupted config updates.
