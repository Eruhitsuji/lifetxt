...
TUI は top-card summary、常時表示の inspector panel、`/` search、theme/keymap 設定、
mark-done、editor 起動、project filter に対応しています。

補足: 最新の Web UI では Display workspace が追加され、light/dark で別paletteを使い、常時表示向けに編集UIを隠しつつ Exit Display ボタンから通常表示へ戻れます。Timeline は `range=today|24h|week` を URL に保存し、選択範囲に dated record がない場合は Next 24h / Week / Items への guided empty state を表示します。選択範囲より前に開始して現在も重なる record は `ongoing` として範囲内に表示します。`+ New` editor では Status / Type / Title / Details に viewport 内に収まる hover/focus help を表示します。
