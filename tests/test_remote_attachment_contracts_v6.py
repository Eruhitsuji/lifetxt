...
class RemoteAttachmentContractTests(unittest.TestCase):
    ...
    def test_web_directory_package_chunk_manifest_and_status(self):
        ...
        client = TestClient(
            webapp.create_app([self.life], writable_path=self.life, config=self.config)
        )
        ...
        response = client.post(
            ...
            json={
                ...
                "attachment_revision": directory_revision(
        ...
        inspected = client.get(
            "/api/attachments/package-manifest",
            params={
                "path": "./bundle.zip",
    ...
    def test_web_clock_guard_capabilities_and_parser_exception(self):
        ...
        client = TestClient(
            webapp.create_app([self.life], writable_path=self.life, config=config)
        )
        capabilities = client.get("/api/capabilities")
        self.assertTrue(capabilities.json()["remote_clock"]["required_for_writes"])
        self.assertEqual(
            "X-Lifetxt-Client-Time",
            capabilities.json()["remote_clock"]["client_time_header"],
        )
        ...
        rejected = client.post(
            ...
            headers={
                "If-Match": revision,
                "X-Lifetxt-Client-Time": "2000-01-01T00:00:00Z",
            },
    ...
    def test_remote_package_source_escape_is_rejected(self):
        ...
        context = McpContext(
            paths=[self.life], writable_path=self.life, config=self.config
        )
        with self.assertRaises(ValueError):
            mcp.call_tool(
                "attachment_package",
                {
                    ...
                    "path": "./bad.zip",
                    "item_revision": mutation.read_text_snapshot(
                        self.life
                    ).content_hash,
                    "attachment_revision": mutation.MISSING_HASH,
