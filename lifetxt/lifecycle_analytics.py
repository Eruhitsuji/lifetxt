...
def lifecycle_analytics(timeline, analysis="summary", timezone_name="UTC"):
    ...
    if analysis == "provenance":
        actors, sources, kinds = Counter(), Counter(), Counter(); missing = 0; source_bounds = {}
        for row in rows:
            ...
            if source:
                sources[source] += 1
                source_bounds.setdefault(source, [row.get("at"), row.get("at")])
                source_bounds[source] = [min(source_bounds[source][0], row.get("at")), max(source_bounds[source][1], row.get("at"))]
            kinds[row.get("record_kind")] += 1
        result.update((("analysis", "provenance"), ("actor_counts", OrderedDict((key, actors[key]) for key in sorted(actors))), ("source_counts", OrderedDict((key, sources[key]) for key in sorted(sources))), ("record_kind_counts", OrderedDict((key, kinds[key]) for key in sorted(kinds))), ("source_first_last", OrderedDict((key, OrderedDict((("first", source_bounds[key][0]), ("last", source_bounds[key][1])))) for key in sorted(source_bounds))), ("missing_provenance_count", missing))); return result
    if analysis in ("schedule", "schedule_lead_time", "due_variance"):
        ...
        if analysis == "schedule":
            ...
            shifts = []
            for row in changes:
                before, after = _date(_payload(row, "before")), _date(_payload(row, "after"))
                if before and after:
                    shifts.append((after - before).days)
            result.update((("analysis", "schedule_revision"), ("field_change_counts", OrderedDict((key, fields[key]) for key in sorted(fields))), ("change_categories", OrderedDict((key, categories[key]) for key in sorted(categories))), ("first_last_values", OrderedDict((key, OrderedDict((("first", first_last[key][0]), ("last", first_last[key][1])))) for key in sorted(first_last))), ("earlier_count", sum(value < 0 for value in shifts)), ("later_count", sum(value > 0 for value in shifts)), ("unchanged_count", sum(value == 0 for value in shifts)), ("net_shift_days", sum(shifts) if shifts else None))); return result
    ...
    if analysis == "progress":
        ...
        result.update((("analysis", "progress_velocity"), ("first_progress", values[0][0] if values else None), ("last_progress", values[-1][0] if values else None), ("total_delta", values[-1][0] - values[0][0] if len(values) > 1 else None), ("elapsed_seconds", elapsed), ("rate_per_day", (values[-1][0] - values[0][0]) / (elapsed / 86400) if elapsed else None), ("positive_delta_count", sum(value > 0 for value in deltas)), ("negative_delta_count", sum(value < 0 for value in deltas)), ("zero_delta_count", sum(value == 0 for value in deltas)), ("largest_deltas", [OrderedDict((("delta", deltas[index]), ("before", _evidence(values[index][1])), ("after", _evidence(values[index + 1][1])))) for index, _ in largest])))
        if not result["complete"]: _limit(result, "history_incomplete")
        return result
    if analysis == "effort":
