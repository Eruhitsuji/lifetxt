...
- [ ] Extend the daily command-center aggregation into the TUI dashboard and Web Dashboard, and add timer/active-session and unacknowledged-message-acknowledgement state once those backends expose it. The shared CLI/MCP aggregation already covers agenda, overdue/due-today, waiting, habits, captures, project attention, and safety warnings.
- [ ] Round out `area:` as core: add area filtering to `filter`/`list` surfaces, shell/Web completion of area values, saved-view integration, and record-level validation. Grouping (`area list|show`), MCP exposure, documentation, and preset examples already exist.
...
- [ ] Extend backlinks/related navigation beyond id references (`parent/ref/depends_on/blocks/related`) to also surface messages, decisions, meetings, people, groups, attachments, projects, milestones, risks, and external URLs. The id-based incoming-link core (`backlinks`, `get_backlinks`) is done.
