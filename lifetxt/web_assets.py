"""The Web page served by :mod:`lifetxt.webapp`.

Extracted from ``webapp.py`` by #82. It is a static asset with a different reason
to change from the routes around it, and at ~9,000 lines it was 75% of that
module.

``webapp`` re-exports this name, and ``surface_runtime`` rebinds
``webapp.HTML_PAGE`` at install time to add the browser revision bridge. The
value here stays the pristine original; read ``lifetxt.webapp.HTML_PAGE`` to get
