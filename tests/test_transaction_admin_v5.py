...
class TransactionAdminTests(unittest.TestCase):
    ...
    def test_policy_write_is_versioned_revision_checked_and_runtime_loaded(self):
        ...
        self.assertEqual(12, policy_from_config({"transactions": {"policy_file": self.policy_path}})["max_transactions"])
        self.assertEqual("alice", read_policy_document(self.policy_path)["updated_by"])
        with self.assertRaises(MutationConflict):
            write_policy_document(self.policy_path, document, expected_revision=MISSING_HASH)
    ...
    def test_legacy_policy_migration_and_newer_refusal(self):
        ...
        result = migrate_policy_file(self.policy_path, expected_revision=revision, operator="bob")
    ...
    def test_audit_is_bounded_and_private(self):
        for index in range(5):
            append_admin_audit(self.audit_path, "event", operator="op", details={"index": index}, max_events=3)
        with open(self.audit_path, "r", encoding="utf-8") as handle:
            record = json.load(handle)
        self.assertEqual([2, 3, 4], [row["details"]["index"] for row in record["events"]])
        if os.name != "nt":
    ...
    def test_archive_rotation_is_dry_run_until_force(self):
        ...
        for index in range(3):
            ...
            os.makedirs(child)
            with open(os.path.join(child, "journal.json"), "w", encoding="utf-8") as handle:
                handle.write("x" * (index + 1))
            with open(os.path.join(child, "integrity-manifest.json"), "w", encoding="utf-8") as handle:
                handle.write("{}")
        ...
        done = rotate_archives(archive, max_archives=1, force=True, audit_file=self.audit_path, operator="admin")
        self.assertTrue(done["ok"])
        self.assertEqual(2, len(done["removed"]))
        self.assertEqual(["tx2"], sorted(name for name in os.listdir(archive) if name.startswith("tx")))
