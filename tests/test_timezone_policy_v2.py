...
class TimezonePolicyV2Tests(unittest.TestCase):
    def test_precedence_cli_file_config_host(self):
        ...
        self.assertEqual("Europe/London", resolve_timezone_name(config, text, "Europe/London"))
    ...
    def test_aware_datetime_converts_to_resolved_zone(self):
        ...
        self.assertEqual((2026, 7, 23, 9, 0), (value.year, value.month, value.day, value.hour, value.minute))
    ...
    def test_dst_fold_requires_explicit_policy(self):
        ...
        self.assertEqual("ambiguous", classify_wall_time(naive, "America/New_York")["state"])
        ...
        self.assertLess(earlier.astimezone(datetime.timezone.utc), later.astimezone(datetime.timezone.utc))

    def test_dst_gap_requires_or_applies_explicit_policy(self):
        naive = datetime.datetime(2026, 3, 8, 2, 30)
        self.assertEqual("nonexistent", classify_wall_time(naive, "America/New_York")["state"])
        with self.assertRaises(TimezonePolicyError):
            interpret_datetime(naive, "America/New_York")
    ...
    def test_time_only_offset_is_anchored_before_conversion(self):
        ...
        self.assertEqual((2026, 7, 23, 18, 15), (value.year, value.month, value.day, value.hour, value.minute))
    ...
    def test_date_boundaries_are_timezone_aware(self):
        ...
        self.assertEqual((23, 59, 59, 999999), (end.hour, end.minute, end.second, end.microsecond))
